    ...
    mutations.forEach((mutation) => {
      if (mutation.addedNodes.length > 0) {
        mutation.addedNodes.forEach((node) => {
          if (node.nodeType === Node.ELEMENT_NODE && node.matches('[data-element-type="p"]')) { // Palabra clave
...
const subjectImages = {
  'Matematicas': 'https://www.masscience.com/wp-content/uploads/2019/10/mat-c.png',
  'Biologia': 'https://cdn-icons-png.flaticon.com/512/5010/5010270.png',
  'Historia y Filosofia': 'https://bbs.hoyolab.com/hoyowiki/picture/object/Philosophies%20of%20Resistance_icon.png'
};
...
function addSubjectArea(subject) {
  ...
  document.getElementById('conversationContainer').appendChild(subjectContainer);
...
function captureMessages() {
  ...
  const areaMentioned = shadowRoot.querySelectorAll('[data-testid="guest-bubble"]');
  ...
  areaMentioned.forEach((el) => {
    ...
    } else if (areaText.includes('Profesor de historia') || areaText.includes('Profesor de filosofia')) {
      latestSubject = 'Historia y Filosofia';
    } else if (!areaText.includes('Profesor de')) { // Validacion extra
  ...
  // Obtener mensajes
  const messages = shadowRoot.querySelectorAll('[data-element-type="variable"]');
  messages.forEach((el) => {
